...
2. **Own words.** Never copy or closely paraphrase a book's sentences or structure (course books such as Yaylagül, McQuail, Baran & Davis …). Facts, names, dates and ideas are free; the wording must be yours. Quotations: very short (<= 15 words), exact, attributed with source id, only when the exact words matter.
...
- The `exam` section is written for university students preparing for exams: the typical question patterns, the tempting distractors, the name/date pairs that get mixed up, and any alternative Turkish names the course books use for the same thing.
